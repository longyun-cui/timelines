    ...
    <div class="col-md-12">
        ...
        <div class="box box-info">
            ...
            <div class="box-header with-border" style="margin:16px 0;">
                <h3 class="box-title">时间线列表</h3>

                <div class="pull-right">
                    <a href="{{url('/home/line/create')}}">
                        <button type="button" onclick="" class="btn btn-success pull-right"><i class="fa fa-plus"></i> 添加时间线</button>
                    </a>
                </div>
            </div>

            <div class="box-body" id="line-list-body">
                <!-- datatable start -->
                ...
                "columns": [
                    {
                        ...
                        render: function(data, type, row, meta) {
                            return '<a target="_blank" href="/line/'+data+'">'+row.title+'</a>';
                        }
                    },
                    {
                        'data': 'visit_num',
                    ...
                    },
                    {
                        ...
                        render: function(data, type, row, meta) {
                            var active_html = '';
                            if(row.active == 1)
                                active_html = '<li><a class="line-disable-submit" data-id="'+data+'">设为私密</a></li>';
                            else
                                active_html = '<li><a class="line-enable-submit" data-id="'+data+'">公开分享</a></li>';
                            ...
                            var html =
                                ...
                                '</button>'+
                                '<ul class="dropdown-menu" role="menu">'+
                                '<li><a href="/home/line/edit?id='+data+'">编辑</a></li>'+
//                                '<li><a href="/home/line/point?id='+data+'">内容管理</a></li>'+
                                active_html+
//                                '<li><a href="/admin/statistics/page?module=2&id='+data+'">流量统计</a></li>'+
//                                '<li><a class="download-qrcode" data-id="'+data+'">下载二维码</a></li>'+
                                '<li><a class="line-delete-submit" data-id="'+data+'" >删除</a></li>'+
                                '<li class="divider"></li>'+
                                '<li><a href="#">Separated link</a></li>'+
                                '</ul>'+
                                '</div>';
...
<script>
    $(function() {
        ...
        // 表格【删除】
        $("#line-list-body").on('click', ".line-delete-submit", function() {
            var that = $(this);
            layer.msg('确定要删除该"时间线"么', {
                time: 0
                ,btn: ['确定', '取消']
                ,yes: function(index){
                    $.post(
        ...
        // 表格【分享】
        $("#line-list-body").on('click', ".line-enable-submit", function() {
            var that = $(this);
            layer.msg('确定公开分享该"时间线"？', {
                time: 0
                ,btn: ['确定', '取消']
                ,yes: function(index){
                    $.post(
        ...
        // 表格【取消分享】
        $("#line-list-body").on('click', ".line-disable-submit", function() {
            var that = $(this);
            layer.msg('确定设为私密该"时间线"？', {
                time: 0
                ,btn: ['确定', '取消']
                ,yes: function(index){
                    $.post(
